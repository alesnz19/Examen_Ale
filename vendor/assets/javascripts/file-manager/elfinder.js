...
elFinder.prototype = {
	...
	notify : function(opts) {
		var type     = opts.type,
			...
			ntpl     = '<div class="elfinder-notify elfinder-notify-{type}"><span class="elfinder-dialog-icon elfinder-dialog-icon-{type}"/><span class="elfinder-notify-msg">{msg}</span> <span class="elfinder-notify-cnt"/><div class="elfinder-notify-progressbar">Please wait...</div>',
			delta    = opts.cnt,
 ...
 * @type String
 **/
elFinder.prototype.version = '2.x (Nightly: 2fe69ed)';
...
/*
 * File: /home/osc/elFinder-build/elFinder/js/elFinder.home.js
 */

/**
 * elFinder home registry.
 * Store shared data
 *
 * @type Object
 * @author Dmitry (dio) Levashov
...
$.fn.dialogelfinder = function(opts) {
	...
	this.not('.elfinder').each(function() {
		...
		var doc     = $(document),
			...
			button  = $('<a href="#" class="dialogelfinder-drag-close ui-corner-all"><span class="ui-icon ui-icon-closethick">&times;</span></a>')
				.appendTo(toolbar)
				.click(function(e) {
					e.preventDefault();
...
$.fn.elfinderbutton = function(cmd) {
	return this.each(function() {
		...
		var c        = 'class',
			...
			menu,
			button   = $(this).addClass('ui-state-default elfinder-button tooltips')
...
$.fn.elfindercontextmenu = function(fm) {
	...
	return this.each(function() {
		var menu = $(this).addClass('ui-helper-reset ui-widget ui-state-default ui-corner-all elfinder-contextmenu fadeIn animated elfinder-contextmenu-'+fm.direction)
				.hide()
				.appendTo('body')
...
$.fn.elfinderdialog = function(opts) {
	...
	this.filter(':not(.ui-dialog-content)').each(function() {
		var self       = $(this).addClass('ui-dialog-content ui-widget-content'),
			...
			dialog = $('<div class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-draggable bounceIn animated std42-dialog  '+cldialog+' '+opts.cssClass+'"/>')
				.hide()
				.append(self)
				.appendTo(parent)
		...
		$.each(opts.buttons, function(name, cb) {
			var button = $('<button type="button" class="ui-button btn btn-xs ui-widget ui-state-default ui-corner-all ui-button-text-only"><span class="ui-button-text">'+name+'</span></button>')
...
$.fn.elfindersearchbutton = function(cmd) {
	return this.each(function() {
		var result = false,
			...
			input  = $('<input type="text" size="42" class="file-search" />')
				.appendTo(button)
		...
		$('<span class="ui-icon ui-icon-search tooltips" title="'+cmd.title+'"></span>')
			.appendTo(button)
			.click(search);
		
		$('<span class="ui-icon ui-icon-close icon-close">&times;</span>')
			.appendTo(button)
			.click(abort)
...
$.fn.elfindersortbutton = function(cmd) {
	...
	return this.each(function() {
		var fm       = cmd.fm,
			...
			asc      = selected+'-asc',
			desc     = selected+'-desc',
			button   = $(this).addClass('ui-state-default elfinder-button tooltips elfinder-menubutton elfiner-button-'+name)
				.attr('title', cmd.title)
			...
			menu = $('<div class="ui-widget ui-widget-content elfinder-button-menu fadeIn animated ui-corner-all"/>')
				.hide()
				.appendTo(button)
				.zIndex(12+button.zIndex())
				.delegate('.'+item, 'hover', function() { $(this).toggleClass(hover) })
 ...
 * Mount network volume with users credentials.
 *
 * @author Dmitry (dio) Levashov
 **/
